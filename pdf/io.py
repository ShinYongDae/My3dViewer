...
def write_codev_gridint(array, filename, comment='CV GRD generated by prysm', typ='SUR', nnb=False):
    ...
    nnb : bool, optional
        ...
        When nnb=False, Code V will use bilinear interpolation between points
        when ray tracing.  This can cause failure when ray-tracing when there
        are high slopes near where the chief ray would intersect the surface.
        When nnb=False, first order implications due to the interferogram are
        more accurate, such as pupil positions.

        When nnb=True, nearest neighbor interpolation between points is used
        when ray tracing.  This avoids potential failure modes due to issues
        finding the chief ray path through the system, but requires higher
        resolution grid int files be provided to well-represent the deformation.
    ...
    hdr = comment + '\n' + f'GRD {n} {m} {typ} WVL 1.0 {nnb}SSZ {scale} NDA -32768\n'
